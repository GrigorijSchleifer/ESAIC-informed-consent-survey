...
### Graph ..: What could be a major advantage of online/telephone interviews?
```{r}
...
map_dataframe <- survey_fragments %>% 
    ...
    summarise(mean_legal = mean(parents.legal.online, na.rm = TRUE)) %>% 
    bind_cols(only_counts_responses) %>% 
    arrange(desc(n))

map_dataframe %>% 
    mutate(country = fct_reorder(as.factor(country), n)) %>% 
    ggplot(aes(x = country, y = mean_legal)) +
    geom_col() +
    labs(title = "Do you know if it is legally allowed to obtain informed consent from the parent/caregiver via Internet or telephone?") +
    coord_flip()
```


```{r echo=FALSE, All questions separated by LimeSurvey}
