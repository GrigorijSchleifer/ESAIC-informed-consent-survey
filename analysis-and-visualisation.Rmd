...
technological <- data.frame(answer = c("Different", "No", "Yes"),
                        when_simple = round(c(0.06647399, 0.70231214, 0.23121387), 2) * 100)
...
#        inset = -.06)
```
```{r Technical analysis countries}
technical_barchart <- survey_data %>% 
    select(c("Country where you work?", 
             "Is it possible to obtain informed consent online via internet, in your routine setting?"))
    
colnames(technical_barchart) <- c("country", "technical")

technical_barchart <- technical_barchart %>% 
    dplyr::filter(technical != "Other") %>% 
    group_by(country) %>% 
    mutate(sum_responses = sum(n)) %>% 
    dplyr::filter(technical == "Yes") %>% 
    mutate(prop_yes = n/sum_responses,
           prop_rest)
    

technical_barchart$country <- reorder(technical_barchart$country, 
                                      technical_barchart$n , 
                                      FUN=function(x) max(as.numeric(x)))

# changing UK name so in can fit to mapdata
technical_barchart$country[technical_barchart$country == "United Kingdom (UK)"] <- "UK" 
technical_barchart$country[technical_barchart$country == "North Macedonia (formerly Macedonia)"] <- "Macedonia" 
technical_barchart$country[technical_barchart$country == "Bosnia and Herzegovina"] <- "B&H" 

# stacked responses base on country
technical_barchart %>%
    dplyr::filter(!is.na(country)) %>%
    dplyr::filter(!is.na(technical)) %>%
    group_by(country) %>% 
    ggplot(aes(x = country, n), fill = technical) +
