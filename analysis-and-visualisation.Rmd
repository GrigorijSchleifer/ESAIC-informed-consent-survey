...
simple <- data.frame(when_obtain_simple_complex <- c("2 days or more",
                        "24h or less",
                        "Same day"),
                     when_simple <- round(c(0.1289111, 0.5419274, 0.3291615), digits = 2) * 100)
...
simple %>% 
    ggplot(aes(x="", y=when_simple, fill=when_obtain_simple_complex)) +
    geom_bar(stat="identity", width=1, color = "white") +
    coord_polar("y", start=0) + 
    theme_void() + 
    geom_text(aes(label = when_simple),
              position = position_stack(vjust = 0.5)) +
    ...
    scale_fill_manual(values=c("#4B72AF", "#54A968", "#C64E51", "#DCAD94"))
```
