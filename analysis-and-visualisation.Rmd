...
#            append = TRUE)
```
```{r Specialty distribution separated by country}
...
### Results
```{r When do you need to obtain Informed consent for elective surgery based on legal requirements?}
...
```
```{r Do you know if it is legally allowed to obtain informed consent from the parent via Internet or telephone?}
...
map_dataframe %>% 
    ...
    coord_flip()
```
```{r "Do you need to obtain written Informed consent for elective surgery due to legal requirements from both parents or just from one?"}
...
cols = brewer.pal(n = length(consent_parents_simple_answers), name = 'Set3')
par(mfrow = c(1, 2))
