...
profession_table_nona <- table(survey_data$Profession, useNA = "no")
profession_table_yesna <- table(survey_data$Profession)
prop.table(profession_table_nona)
prop.table(profession_table_yesna)
```
...
#            append = TRUE)
```
```{r Plot Specialty distribution separated by country (images saved)}
...
survey_data_barchart <- survey_data_barchart %>% 
    dplyr::filter(expert.level != "Other")
...
survey_data_barchart %>%
    ...
    labs(x = "",
         y = "Responses") +
    # "#a6bddb", "#67a9cf", "#3690c0", "#02818a", "#016c59"
    scale_fill_manual(values=c("#2171b5", "#08306b", "#c6dbef", "#6baed6", "#f7fbff"))
...
### Results
```{r PIE Do you know if it is legally allowed to obtain informed consent from the parent via Internet or telephone (saved) ?}
...
table(survey_data$"Do you know if it is legally allowed to obtain informed consent from the parent/caregiver via Internet or telephone?")

# "#a1dab4", "#41b6c4", "#2c7fb8", "#253494" 2 (Claudia bestätigt)
# scale_fill_manual(values=c("#a1dab4", "#41b6c4", "#2c7fb8", "#253494"))

online_legal_ped_answers <- data.frame(answer = c("Depends on the procedure", "Yes", "No"),
...
#        inset = -.06)
```
```{r MAP Do you know if it is legally allowed to obtain informed consent from the parent via Internet or telephone? (saved)?}
...
map_data %>% 
    ...
    theme(axis.text.x = element_blank(),
          ...
          legend.title = element_blank(),
          rect = element_blank()) +
    scale_fill_gradient(low = "#5591D7", high = "#1F334E") 
...
map_dataframe %>% 
    ...
    coord_flip()
```
```{r PIE Do you need to obtain written Informed consent for elective surgery from both parents or just from one? (saved)}
...
table(survey_data$"Do you need to obtain written Informed consent for elective surgery due to legal requirements from both parents or just from one (simple procedures, low risk patients)?")
...
table(survey_data$"Do you need to obtain written Informed consent for elective surgery due to legal requirements from both parents or just from one (complex procedures, higher risk patients)?")

both_consent <- data.frame(simple_complex = c(rep("simple", 3), rep("complex", 3)),
                        answer = rep(c("Both parents", "Depends on the procedure", "One, who is present"), 2),
                        when_simple = round(c(0.14219474, 0.08191654, 0.77588872, 0.2569231, 0.1046154, 0.6384615), digits = 3) * 100)
...
id_dilemma_tbl <- data.frame(Values = c(49, 234, 362, 29, 423, 193),
                             Answer = rep(c("Depends on the procedure", "No", "Yes"), 2),
...
table(survey_data$"How would you prefer to do the anaesthesiological preoperative evaluation in the future?")
...
future_consent <- data.frame(answer = c("In person",
                                       "Self assessment online",
                                       "Videoconference",
                                       "Telephone"), 
                             counts = round(c(0.77910053, 0.07539683, 0.11640212, 0.02910053), 3) * 100)

future_consent_ped <- data.frame(answer = c("Depends on the procedure", "No", "Yes"),
                                 counts = round(c(0.0755814, 0.3226744, 0.6017442), 3) * 100)


future_consent_plot <- future_consent %>% 
    ggplot(aes(x="", y=counts, fill=answer)) +
    geom_bar(stat="identity", width=1, color = "white") +
    coord_polar("y", start=0) + 
    theme_void() + 
...
future_consent_ped_plot <- future_consent_ped %>% 
    ggplot(aes(x="", y=counts, fill=answer)) +
    geom_bar(stat="identity", width=1, color = "white") +
    coord_polar("y", start=0) + 
    theme_void() + 
...
ggarrange(future_consent_plot, 
          future_consent_ped_plot, 
          labels = c("A:  How would you prefer to do the anaesthesiological preoperative evaluation in the future?", 
                     "B:  When ensuring certain guidelines, would remote interviews be an alternative to personal face-to-face meetings?"),
...
# 24 h prior and 24 prior because of daily routine pooled together
technological <- data.frame(answer = c("Depends on the procedure", "Yes", "No"),
...
#        inset = -.06)
```
```{r MAP Technical analysis }
technical_map <- survey_data %>% 
    select(c("Country where you work?", 
             "Is it possible to obtain informed consent online via internet, in your routine setting?"))
    
colnames(technical_map) <- c("country", "technical")
    
# get counts of responses for every contry
only_counts_responses <- survey_fragments %>% 
    dplyr::filter(country != "Other") %>% 
    count(country) %>% 
    select(n)
...
technical_map <- technical_map %>% 
    dplyr::filter(country != "Other") %>% 
    select(country, technical) %>% 
    group_by(country) %>% 
    mutate(technical_possible = ifelse(technical == "Yes", 1, 0)) %>% 
    summarise(mean_technical = (mean(technical_possible, na.rm = T)) * 100) %>% 
    bind_cols(only_counts_responses) %>%
    dplyr::filter(n > 15) %>% 
    arrange(desc(n))
...
# changing UK name so in can fit to mapdata
technical_map$country[technical_map$country == "United Kingdom (UK)"] <- "UK" 

# getting the countries in R
mapdata <- map_data("world")
# changing column names, so mapdata and map_dataframe can be combined
colnames(technical_map) <- c("region", "mean_technical", "n")
# changing UK name so in can fit to mapdata
#combining both dataframes
technical_map_joined <- left_join(mapdata, technical_map, by = "region")


technical_map_joined %>% 
    dplyr::filter(!is.na(mean_technical)) %>% 
    ggplot(aes(x = long, 
               y = lat, 
               group = group)) +
    geom_polygon(aes(fill = mean_technical), 
                 color = "black") +
    theme(legend.position = "bottom",
          legend.key.height= unit(1, 'cm'),
          legend.key.width= unit(2, 'cm'),
          ...
          axis.title.x=element_blank(),
          rect = element_blank()) +
    scale_fill_gradient(low = "#F5F5F5", high = "#438C67") 
```
...
```{r Table Questionaires items pediatric}
Questionaire_items = c(
...
Questionaire_items <-
    CreateTableOne(
        data = survey,
        vars = contVars_Questionaire_items,
        # factorVars = factorVars,
        addOverall = FALSE,
        # strata =
    )

results_tbl <- print(Questionaire_items, nonnormal = biomarkers, exact = "stage", quote = FALSE, noSpaces = FALSE, printToggle = FALSE)
```
